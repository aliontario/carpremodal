...
# 1. Saved model aur columns ko load karna
...
# 2. Streamlit App UI Setup
st.title("🚗 Used Car Price Prediction App")
st.write("Enter the car features to predict its estimated selling price:")

# User Inputs
present_price = st.number_input("Present Showroom Price (in Lakhs, e.g., 5.5)", min_value=0.1, max_value=100.0, value=6.0)
kms_driven = st.number_input("Kilometers Driven", min_value=0, max_value=500000, value=30000)
car_age = st.number_input("Age of the Car (in Years)", min_value=0, max_value=30, value=5)
...
if st.button("Predict Selling Price"):
    ...
    # DataFrame creation to match model columns order
